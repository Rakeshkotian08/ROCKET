...
void mySpecialKeyFunc( int key, int x, int y )
{
   ...
   case GLUT_KEY_UP:
      if ( AnimateStep < 5.0) {         // Don't let speed get very big.
         AnimateStep *= sqrt(3.0);      // Increase the angle increment
      }
      break;
   case GLUT_KEY_DOWN:
...
void drawScene(void)
{
   ...
   if ( LightIsPositional==1 ) {
      glLightfv(GL_LIGHT2, GL_POSITION, zeroPos );   // Position is transformed by ModelView matrix
   }
   else {
      glLightfv(GL_LIGHT2, GL_POSITION, dirI );      // Direction is transformed by ModelView matrix
   }
   glPopMatrix();
...
void initRendering()
{
   ...
   glEnable(GL_LIGHT2);      // Turn on light #0.

   // Set global ambient light
   glLightModelfv(GL_LIGHT_MODEL_AMBIENT, ambientLight);

   // Light 0 light values.  Its position is set in drawScene().
   glLightfv(GL_LIGHT2, GL_AMBIENT, Lt0amb);
   glLightfv(GL_LIGHT2, GL_DIFFUSE, Lt0diff);
   glLightfv(GL_LIGHT2, GL_SPECULAR, Lt0spec);
}
